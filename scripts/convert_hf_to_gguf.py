...
class NemotronNanoV2VLModel(MmprojModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if "patch_generator.pos_embed" in name:
            ...
            target_patches_per_side = image_size // patch_size
            max_patches_per_side = int((data_torch.shape[1]) ** 0.5)
...
class Gemma3Model(TextModel):
    ...
    def set_gguf_parameters(self):
        ...
        self.gguf_writer.add_rope_freq_base(self.rope_parameters.get("full_attention", self.rope_parameters).get("rope_theta", 1_000_000.0))
...
class Gemma3nVisionAudioModel(ConformerAudioModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if new_name.endswith("conv_stem.conv.bias") or new_name.endswith("layer_scale.gamma"):
            data_torch = data_torch.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
...
class GroveMoeModel(TextModel):
    ...
    def modify_tensors(self, data_torch: Tensor, name: str, bid: int | None) -> Iterable[tuple[str, Tensor]]:
        ...
        if name.find("chunk_experts") != -1:
            n_experts = self.find_hparam(["num_local_experts", "num_experts"]) // 2
            assert bid is not None
...
class MistralModel(LlamaModel):
    ...
    def set_mistral_config(gguf_writer: gguf.GGUFWriter, hparams: dict):
        if "yarn" in hparams:
            ...
            gguf_writer.add_rope_scaling_yarn_log_mul(1.0)
