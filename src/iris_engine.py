...
_hw_thread.Thread(
    target=lambda: __import__("src.hardware_profile", fromlist=["summary"]).summary(),
    daemon=True,
).start()
...
def _stream_tokens(
    ...
    # For large/max web design requests: swap CODE prompt to free-form creative brief.
    # Only applies when no explicit system_prompt_override was given (e.g. reviewer passes
    # its own override, which must not be replaced).
    if (
        system_prompt_override is None
        and role == ModelRole.CODE
        and isinstance(settings, dict)
        and settings.get("_large_creative_mode")
    ):
        try:
            from src.iris_coding import get_large_code_prompt
            sys_prompt = get_large_code_prompt(IRIS_IDENTITY)
        except Exception as _lcp_e:
            logger.warning(f"[Iris] Failed to load large_prompt.txt, falling back to coding_prompt.txt: {_lcp_e}")
    ...
    for loop_idx in range(5):
        ...
        if _ca_cfg.get("enabled", False) and len(full_messages) > 4:
            ...
            _compressed = smart_compress(
                ...
                query=_query,
                n_ctx=llm.n_ctx(),
                max_output_tokens=min(max_tokens, _reserved_tokens)
                if max_tokens
